...
ATTENTION: This email came from an external source. Do not open attachments or click on links from unknown senders or unexpected emails. Please click on the “Phish Alert” button on the top right of the Outlook toolbar to report any suspicious emails.
Entire CCD:
"""
...
INPUT CSV:
  - PROD must contain both "bucket" and "key" columns from the Athena export.
  - DEV may omit "bucket" and use the configured default learning bucket.
  - Typically produced by:
    * makelistofdevdocsfollowingprodcsvformat.py (DEV)
    * Athena query export (PROD)
...
# ============================================================================
# PROD PROFILE — bucket-aware Athena input
# ============================================================================
# Each PROD CSV row must identify its actual S3 bucket. Both the primary and
# -part2 buckets are accepted for the six production QEs.

PROD = {
    "aws_profile": "default",
    "default_bucket": None,  # Do not silently assume Hixny in PROD
    "allowed_buckets": [
        "nyec-pdr-prod-hixny",
        "nyec-pdr-prod-hixny-part2",
        "nyec-pdr-prod-techbd",
    ...
    "max_files": 30000,  # None = process all documents
}
...
def read_input_csv_file(
    ...
    Returns:
        list of dict: Each item contains bucket, key, and full s3:// path.
    """
    documents = []
    allowed_buckets = set(allowed_buckets or [])
    ...
    with open(csv_path, "r", encoding="utf-8-sig", newline="") as csvfile:
        ...
        # Athena emits lowercase headers, but tolerate casing and whitespace.
        header_map = {name.strip().lower(): name for name in reader.fieldnames}
        key_column = header_map.get("key")
        bucket_column = header_map.get("bucket")
        ...
        for row_number, row in enumerate(reader, start=2):
            ...
            # Also accept complete S3 URIs in the key column.
            if raw_key.lower().startswith("s3://"):
                s3_location = raw_key[5:]
                parsed_bucket, separator, parsed_key = s3_location.partition("/")
                if not separator or not parsed_key:
                ...
                if bucket and bucket != parsed_bucket:
                    raise ValueError(
                        f"Bucket mismatch on CSV row {row_number}: "
                        f'column says "{bucket}" but key URI says "{parsed_bucket}"'
                    )
...
def main():
    ...
    try:
        # Read ALL locations from the CSV. max_files is applied after restart
        # filtering so each run receives the next unprocessed batch.
        xml_documents = read_input_csv_file(
            INPUT_CSV,
            default_bucket=DEFAULT_BUCKET,
            allowed_buckets=ALLOWED_BUCKETS,
        ...
        print(f"  [OK] Found {len(xml_documents)} total CCD documents in input CSV")
        bucket_counts = Counter(doc["bucket"] for doc in xml_documents)
        for bucket, count in sorted(bucket_counts.items()):
            print(f"       {bucket}: {count}")
    except Exception as e:
    ...
    if already_processed:
        ...
        remaining_documents = [
            ...
            if document["path"] not in already_processed
        ]

        skipped_count = len(xml_documents) - len(remaining_documents)
        print(f"       Skipping {skipped_count} already-processed files")
        print(f"       {len(remaining_documents)} files remaining to process")
    ...
    print("  Buckets:     Selected per input CSV row")
    ...
    for file_index, document in enumerate(xml_documents, 1):
        ...
        if len(results) % 200 == 0:
            _flush_results_to_csv(results, already_processed)
            already_processed.update(result["Path"] for result in results)
            results.clear()
    ...
    total_elapsed_ms = int((time.time() - run_start_time) * 1000)
    total_processed_this_run = len(xml_documents)
...
Syracuse, NY 13204
druppert@healtheconnections.org
